	...
	public void paint(Graphics g) {
		try {
			...
			if(checkerboard.getCursor_position()[0] != -1 && checkerboard.getCursor_position()[1] != -1) {
				...
				g.drawImage(ImageIO.read(new File(checkerboard.getCURSOR_IMAGE_URL())), cursorX, cursorY, this);
			}
			...
			for(int i = 0; i < checkerboard_situation.length; i++) {
				for(int j = 0; j < checkerboard_situation.length; j++) {
					if(checkerboard_situation[i][j] == 1) {
						g.drawImage(ImageIO.read(new File(checkerboard.getBLACK_CHESS_PIECES_IMAGE_URL())), //棋子图片
						...
						g.drawImage(ImageIO.read(new File(checkerboard.getWHITE_CHESS_PIECES_IMAGE_URL())), 
